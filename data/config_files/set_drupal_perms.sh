...
chmod -R u=rw,go= .gitignore

printf "Changing permissions of various Drupal text files in \"${drupal_path}\" to \"rwx------\"...\n"
cd ${drupal_path}
chmod -R u=rw,go= CHANGELOG.txt
chmod -R u=rw,go= COPYRIGHT.txt
chmod -R u=rw,go= INSTALL.mysql.txt
chmod -R u=rw,go= INSTALL.pgsql.txt
chmod -R u=rw,go= INSTALL.txt
chmod -R u=rw,go= LICENSE.txt
chmod -R u=rw,go= MAINTAINERS.txt
chmod -R u=rw,go= UPGRADE.txt
